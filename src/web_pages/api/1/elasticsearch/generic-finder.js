...
//FIXME foo, bar
			...
			processData: false,
			//dataType: 'json',
			success: function(token, status_target) { return function(ajaxdata, status, xhr) {
			...
			error: function(token, status_target) { return function(xhr, status, error) {
				if(xhr.status == 503) {
					cb('slow');
				}
				else {
...
//FIXME
		...
		var ajx = $.ajax({
			url: url,
			type: 'POST',
			data: JSON.stringify({'items': items, 'auth_token': token}),
			processData: false,
			dataType: 'json',
			success: function(data, scroll_id, count, size, total, cart_id, items, token, email_addr) { return function(ajaxdata, status, xhr) {
				if(!cart_id) {
...
//FIXME what value here? 2 gigs for now.
			...
			error: function(data, cart_id) { return function(xhr, text_status, error) {
...
//FIXME Better UI.
		...
		data['widget_data']['cart_cancel'] = {'cart_id': cart_id, 'ajax': ajx};
	}
	function download_scan(data, scroll_id, count, size, cart_id, email_addr) {
		var ajx = $.ajax({
			...
			processData: false,
			dataType: 'json',
			success: function(data) { return function(ajaxdata, status, xhr) {
				var items = [];
			...
			error: function(data, cart_id) { return function(xhr, text_status, error_thrown) {
				if(text_status != 'abort') {
//FIXME Better UI.
					alert(xhr.responseText);
					data['widget_data']['cart_cancel'] = null;
	...
	var widgets = {
		'simple_items': {
			...
			'after_load': function(target, data) {
				...
				}).click(function(data) { return function() {
				...
				$.ajax({
					...
					success: function(data) { return function(xml) {
...
//FIXME look up icon service url.
				...
				e.find('.myemsl_iteminfo').click(function(item_id, source) { return function(e) {
...
//FIXME This never gets cleaned up.
				...
				l.click(function(item_id, filename, renamed_data) { return function(e) {
					var ajx = $.ajax({
//FIXME service file.
						...
						processData: false,
						dataType: "text",
						success: function(item_id, file, slow_dialog) { return function(data, status, xhr) {
							var server = "";
...
//FIXME foo, bar //Escape url
							myemsl_tape_status(data, id, function(server, id, token) { return function(state) {
								var cb = function() {
...
//				e.find('.myemsl_search_proposal_id').append(l);
			...
			'process_done': function(container, data, hit, query) {
				...
				if('to_process' in widget_data) {
					widget_data['slow_timer'] = setTimeout(function(container, data, to_process) { return function() {
						...
						for(var i in to_process) {
							...
							var ajx = myemsl_tape_status(token, id, function(status) { return function(state) {
								if(state != "fast") {
				...
				widget_data['query_cart_download'] = function(data, query) { return function() {
					...
					var newquery = {query: $.extend({}, query['query'])};
					$.ajax({
...
//							} 
		...
		'unit': {
			...
			'process_row': function(container, data, hit, id) {
				...
				var e = $('<tr><td class="myemsl_search_proposal_id"></td><td>' + title + '</td></tr>');
				var l = $('<a href="#">' + symb + '</a>');
				l.click(function(data, id, type, namep) { return function(e) {
					data.proposal_clicked({'key': namep, 'val': id, 'type': type});
					e.preventDefault();
				}}(data, id, type, title));
				e.find('.myemsl_search_proposal_id').append(l);
				return e;
...
//				target.css('overflow', 'auto');
				...
				section.find('a').click(function(target, data) { return function (e) {
					e.preventDefault();
...
//								alert(name);
								data.proposal_clicked({'key': name, 'val': '(unknown:' + name + ')', 'type': 'unknown'});
								$(this).dialog("close");
							},
							Cancel: function() {
								$(this).dialog("close");
...
//				}}(target));
		...
		'predicate': {
			...
			'process_row': function(container, data, hit, id) {
				...
				var l = $('<a href="#">' + id + '</a>');
				l.click(function(data, id) { return function(e) {
					data.proposal_clicked({'key': hit['_source']['description']['short'], 'val': id});
					e.preventDefault();
				}}(data, id));
				...
				}).click(function(hit) { return function() {
	...
	var methods = {
			...
			return this.each(function() {
				...
				if(!data) {
					var data = {
						...
						'display_facets_update': function(display_facets, data) {
							display_facets.sort(function(a, b) {return data.facet_desc[a.id] > data.facet_desc[b.id]});
						},
						'process_done': null,
						'after_load': null,
						facets_set: [],
						...
						old_query: null,
						state_save: false,
						proposal_clicked: function($this) { return function(id) {
							var data = $this.data('myemslProposalFinderForm');
							data.proposal = id;
					...
					$this.load(data.layout_url, function(data, $this) { return function () {
						...
						data['myemsl_search_pager'].myemslSearchPager({
							'switch_page': function($this, data) { return function(page) {
		...
		facet_link_set: function(args) {
			...
			return this.each(function() {
				var $this = $(this);
				var data = $this.data('myemslProposalFinderForm');
				var a = {'facet':facet, 'term':term, 'query_term':query_term};
...
/*FIXME This is really not the place for this. */
				...
				for(var i in data.facets_set) {
					...
					l.click(function(target, facet, term) { return function(e) {e.preventDefault(); target.myemslProposalFinderForm('facet_link_remove_to', {'facet':facet, 'term':term});}}(target, facet, term));
					var l2 = $('<a href="#" class="myemsl_search_selected_facet_remove_link">(X)</a>');
					l2.click(function(target, facet, term) { return function(e) {e.preventDefault(); target.myemslProposalFinderForm('facet_link_remove', {'facet':facet, 'term':term});}}(target, facet, term));
				...
				var config = {'search_url': data.search_url};
...
//FIXME make style better.
								...
								for (var j in facet[region]) {
									...
									l.click(function(target, facet, term, query_term) { return function(e) {
										e.preventDefault();
										target.myemslProposalFinderForm('facet_link_set', {'facet':facet, 'term':term, 'query_term':query_term});
...
//FIXME put an upper bound on what count we allow here.
								...
								if(display_facet['other'] > 0) {
									...
									ddd.click(function(data, display_facet) { return function(e) {
